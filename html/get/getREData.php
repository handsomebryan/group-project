...
if ($gender != '' && $postalCode != '' && $age != '') {
  ...
  $sql = "SELECT `商品資料`.`商品中文名稱` ,`保單資料`.`商品英文代碼`  , COUNT(*) as `購買次數`
    FROM `客戶資料`
    JOIN `保單要被保人` ON `客戶資料`.`客戶序號` = `保單要被保人`.`要保人序號`
    JOIN `保單資料` ON `保單要被保人`.`保單序號` = `保單資料`.`保單序號`
  ...
  $sql = "SELECT `商品資料`.`商品中文名稱` ,`保單資料`.`商品英文代碼`  , COUNT(*) as `購買次數`
    ...
    JOIN `保單資料` ON `保單要被保人`.`保單序號` = `保單資料`.`保單序號`
    JOIN `商品資料` ON `保單資料`.`商品英文代碼` = `商品資料`.`商品英文代碼`
    WHERE `客戶性別` = '$gender' AND `客戶郵遞區號` = '$postalCode' 
    GROUP BY `商品資料`.`商品中文名稱`
    ORDER BY `購買次數` DESC
  ...
  $sql = "SELECT `商品資料`.`商品中文名稱` ,`保單資料`.`商品英文代碼`  , COUNT(*) as `購買次數`
    ...
    JOIN `保單資料` ON `保單要被保人`.`保單序號` = `保單資料`.`保單序號`
    JOIN `商品資料` ON `保單資料`.`商品英文代碼` = `商品資料`.`商品英文代碼`
    WHERE `客戶性別` = '$gender' AND `客戶年齡` BETWEEN '$age' - 5 AND '$age' + 5
    GROUP BY `商品資料`.`商品中文名稱`
    ORDER BY `購買次數` DESC
  ...
  $sql = "SELECT `商品資料`.`商品中文名稱` ,`保單資料`.`商品英文代碼`  , COUNT(*) as `購買次數`
    ...
    JOIN `保單資料` ON `保單要被保人`.`保單序號` = `保單資料`.`保單序號`
    JOIN `商品資料` ON `保單資料`.`商品英文代碼` = `商品資料`.`商品英文代碼`
    WHERE  `客戶郵遞區號` = '$postalCode' AND `客戶年齡` BETWEEN '$age' - 5 AND '$age' + 5
    GROUP BY `商品資料`.`商品中文名稱`
    ORDER BY `購買次數` DESC
    LIMIT 5;"; // SQL for postal code and age
}elseif ($gender != '') {
  // New code for search by gender only
  $sql = "SELECT `商品資料`.`商品中文名稱` ,`保單資料`.`商品英文代碼`  , COUNT(*) as `購買次數`
    FROM `客戶資料`
    JOIN `保單要被保人` ON `客戶資料`.`客戶序號` = `保單要被保人`.`要保人序號`
    JOIN `保單資料` ON `保單要被保人`.`保單序號` = `保單資料`.`保單序號`
  ...
  // New code for search by age only
  $sql = "SELECT `商品資料`.`商品中文名稱` ,`保單資料`.`商品英文代碼`  , COUNT(*) as `購買次數`
    FROM `客戶資料`
    JOIN `保單要被保人` ON `客戶資料`.`客戶序號` = `保單要被保人`.`要保人序號`
    JOIN `保單資料` ON `保單要被保人`.`保單序號` = `保單資料`.`保單序號`
    JOIN `商品資料` ON `保單資料`.`商品英文代碼` = `商品資料`.`商品英文代碼`
    WHERE `客戶年齡` BETWEEN '$age' - 5 AND '$age' + 5
    GROUP BY `商品資料`.`商品中文名稱`
    ORDER BY `購買次數` DESC
    LIMIT 5;";
} elseif ($postalCode != '') {
  // New code for search by postal code only
  $sql = "SELECT `商品資料`.`商品中文名稱` ,`保單資料`.`商品英文代碼`  , COUNT(*) as `購買次數`
    FROM `客戶資料`
    JOIN `保單要被保人` ON `客戶資料`.`客戶序號` = `保單要被保人`.`要保人序號`
    JOIN `保單資料` ON `保單要被保人`.`保單序號` = `保單資料`.`保單序號`
