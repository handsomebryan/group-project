...
if (isset($_GET['total_statistic'])) {
    $visit = "SELECT DATE_FORMAT(DATE(a.拜訪日期), '%Y-%m') AS 日期, COUNT(*) AS 拜訪次數
            ...
            WHERE ac.業務員序號 like '%$s_id' AND ac.客戶序號 like '%$c_id' AND a.拜訪日期 >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR)
            GROUP BY 日期";

    $contact = "SELECT  DATE_FORMAT(DATE(a.聯絡日期), '%Y-%m') as 日期, COUNT(*) as 聯絡次數
                FROM CRM聯繫資料 as a
                JOIN CRM客戶資料 as ac ON a.客戶系統編號 = ac.客戶系統編號
                WHERE ac.業務員序號 LIKE '%$s_id' AND ac.客戶序號 LIKE '%$c_id' AND a.聯絡日期 >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR)
                GROUP BY 日期";
}else{
