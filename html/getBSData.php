...
if ($quarter) {
    ...
    $sql = "WITH TopProducts AS (
                SELECT 
                    商品英文代碼, SUM(年化保費) as 銷售額
                FROM 
                    保單資料
                WHERE 
                    YEAR(保單生效日) = '$year' AND QUARTER(保單生效日) = '$quarter'AND 年化保費 != 0
                GROUP BY 
                    商品英文代碼
                ORDER BY 
                    銷售額 DESC
            ...
            SELECT 
                TP.商品英文代碼, MONTH(保單生效日) as m, SUM(年化保費) as 銷售額
            FROM 
                保單資料 PD
            JOIN 
                TopProducts TP ON PD.商品英文代碼 = TP.商品英文代碼
            WHERE 
                YEAR(保單生效日) = '$year' AND QUARTER(保單生效日) = '$quarter'
            GROUP BY 
                TP.商品英文代碼, MONTH(保單生效日)
            ORDER BY 
                TP.商品英文代碼, MONTH(保單生效日)
            ";
} elseif ($month) {
    ...
    $sql = "WITH TopProducts AS (
                ...
                FROM 
                    保單資料
                WHERE 
                    YEAR(保單生效日) = '$year'and MONTH(保單生效日)= '$month' AND 年化保費 != 0
                GROUP BY 
                    商品英文代碼
                ORDER BY 
                    總銷售額 DESC
